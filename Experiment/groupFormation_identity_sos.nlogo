...
to-report compute-sigmoid [x xOffset steepness repulsion]
  ifelse repulsion = false [
    ...
    report precision (1 / (1 + exp (steepness * (x - xOffset)))) 3
  ][
    ;;sigmoid that outputs from 1 to -1 as x increases from 0 to 1
    report precision (2 / (1 + exp (steepness * (x - xOffset))) - 1) 3
  ]
end
...
to change-opinion-HK
  ...
  if count influentials > 1 [
    ...
    while [step < updating] [
      ...
      ;;ad 1: computing (identity-weighted) mean opinion
      let val precision (mean [item i opinion-position * current-opinion-weight] of influentials) 3 ;; NOTE: H-K model really assumes that agent adopts immediatelly the 'consesual' position

      ;; computing unweighted incoming average for Silence response
      let uval 0
      ifelse graded-identity? = true and identity_type = "individual"[
