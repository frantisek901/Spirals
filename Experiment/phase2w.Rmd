...
phase2w = read_csv("experiment01part41.csv", skip = 6) %>% 
  ...
  add_row(read_csv("experiment01part42b.csv", skip = 6) %>% 
            select(2, 3, identity = 4, 5, 6, step = 48, ESBG = ESBSG_polarization_final)) %>%
  ## TO-DO: Something is wrong with the following hash-out file/code -- find and kill the bug!
  #  Ajajajajajaj..... I rewrite file Part51b.csv mistakenly over the Part55.csv... 
  #  Now I started simulations on Part51b.csv again with right name, and I will try to restore Part55.csv...
  #  Trying...
  ...
  add_row(read_csv("experiment01part51.csv", skip = 6) %>% 
            select(2, 3, identity = 4, 5, 6, step = 48, ESBG = ESBSG_polarization_final)) %>%
  add_row(x= read_csv("experiment01part51b.csv", skip = 6) %>% 
            select(2, 3, identity = 4, 5, 6, step = 48, ESBG = ESBSG_polarization_final)) %>%
