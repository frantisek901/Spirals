...
turtles-own [Opinion-position P-speaking Speak? Uncertainty Record Last-opinion
 ...
 incoming-average-opinion perceived-identity-groups perceived-centroids current-opinion-weight]
...
to set-individual-group-identities
  foreach range identity_levels[ i ->
    ...
    ;;print(word "There are " count centroids " centroids among us.")


    ;; Precomputing perceived identity groups and centroids
    let pig []
    let pc []

    if graded-identity? = true[
      foreach sort [who] of centroids [j ->
        set pig lput sort agents with [group = j] pig
        set pc lput [Opinion-position] of centroids with [who = j] pc
      ]
    ...
    ;; Saving perceived identity groups as agent-sets
    ask agents with [id-threshold-level = i] [
      ifelse (graded-identity? = false)[
        set Identity-group agents with [group = [group] of myself]  ;;own group -> old implementation, can be removed?
        if count Identity-group < 3 [set Identity-group agents]
      ][
        set perceived-identity-groups pig
        set perceived-centroids pc
        ;;show perceived-centroids
      ]
    ]
...
to preparing-myself
  ...
  set speak? speaking
  set incoming-average-opinion n-values opinions [0]
  if silence? [implement-silence]
  getColor
  getPlace
...
to-report compute-sigmoid [x xOffset steepness repulsion]
  ifelse repulsion = false [
    ...
    report precision (1 / (1 + exp (steepness * (x - xOffset)))) 1
  ][
    ;;sigmoid that outputs from 1 to -1 as x increases from 0 to 1
    report precision (2 / (1 + exp (steepness * (x - xOffset))) - 1) 1
  ]
end


;; Silence module for SoS
to implement-silence

 if opinion-distance3 (incoming-average-opinion) (opinion-position) > boundary * silence-boundary-multiplier
 [
   set speak? false
 ]
...
to change-opinion-HK
  ...
  let visibles []
  ;; When identity is ungraded or absolute: 1) updating agent uses only visible link neighbors in small-world network who are also members of her Identity group
  ifelse graded-identity? = false [
    set visibles (turtle-set filter [vis -> member? vis Identity-Group] sort comm-neighbors with [speak?])
  ][
    ;; with graded identity we need to consider ALL influencers regardless of identity
    set visibles (turtle-set sort comm-neighbors with [speak?])
  ]

  ifelse graded-opinions? = true[
    set influentials visibles
  ][
  ...
  ;;Store in current-opinion-weight: opinion weights of each influencer according to the listener's weighting of perceived identity group
  ask influentials[
    ifelse use_identity? = true and identity_type = "individual" and graded-identity? = true [
      set current-opinion-weight get-opinion-weight-by-identity self myself
      ;;show current-opinion-weight
    ][
      set current-opinion-weight 1
    ]
  ]

  if graded-opinions? = true [
    ask influentials[
    ;; if we have graded opinion effects, the current opinion weight is multiplied by the weight due to opinion space distance
      set current-opinion-weight (current-opinion-weight) * (get-opinion-weight-by-opinion self myself)
      ;;show current-opinion-weight
    ]
  ]
  ...
  if count influentials > 1 [
    ...
    while [step < updating] [
      ...
      ;; computing unweighted incoming average for Silence response
      let uval 0
      set uval precision (mean [item i opinion-position] of influentials) 3 ;; NOTE: H-K model really assumes that agent adopts immediatelly the 'consesual' position
      set incoming-average-opinion replace-item i incoming-average-opinion uval
...
to-report get-opinion-weight-by-opinion [influencer listener]
  let myPosition [opinion-position] of listener
  let theirPosition [opinion-position] of influencer
  let opinionDistance opinion-distance3 (theirPosition) (myPosition)
  let HKboundary 0
  if mode = "openly-listen"[
    set HKboundary [Uncertainty * sqrt(opinions * 4)] of listener]
  if mode = "vaguely-speak"[
    set HKboundary [Uncertainty * sqrt(opinions * 4)] of influencer]

  report compute-sigmoid (opinionDistance) (HKboundary) (opinion-sigmoid-steepness) (opinion-repulsion?)
end

to-report get-opinion-weight-by-identity [influencer listener]

  ;;NOTE: This can be made more efficient by pre-computing ALL weights for a given centroid as a single value. Would have to rewrite this.
  let myIdentityPosition []
  let theirIdentityPosition []
  let opinionDistance 0

      let indx 0 ;;index for next loop
      foreach [perceived-identity-groups] of listener [
        grp ->
        ;;show grp
        if member? influencer grp[
          set theirIdentityPosition item 0 item indx [perceived-centroids] of listener   ;; the 'item 0' is to remove an unnecessary extra nesting
        ]
        if member? listener grp[
          set myIdentityPosition item 0 item indx [perceived-centroids] of listener   ;; the 'item 0' is to remove an unnecessary extra nesting
    ]

        set indx indx + 1
      ]
  set opinionDistance opinion-distance3 (theirIdentityPosition) (myIdentityPosition)

  report compute-sigmoid (opinionDistance) (identity-xoffset) (identity-sigmoid-steepness) (identity-repulsion?)
end
