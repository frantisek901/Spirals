...
turtles-own [own-opinion own-previous-opinion own-boundary own-conformity own-SPIRO own-WhichGroupHasEachSPIROSortedMeIn own-group-number own-distance-to-centroid
  own-opinion-sigmoid-xOffset own-opinion-sigmoid-steepness own-opinion-dice? own-identity-sigmoid-xOffset own-identity-sigmoid-steepness own-identity-dice? nei-size discussion-capacity]
...
to setup
  ...
  ifelse not create_permanent_partners? [ask links [die]][ask links [set hidden? TRUE]]
...
to change-opinion-HK
  ;; We define all other agents as NEIGHBS, i.e. potential INFLUENTIALS, in our traditional model,
  ...
  let neighbs rnd:weighted-n-of called_N other agents [((1 - opinion-distance (ifelse-value (Use_Present_Opinion?) [own-opinion][own-previous-opinion]) ([own-opinion] of myself) (false)) * delta) ^ alpha]
  ...
  ;; Note: Twitter agents have to pass the 'boundary check'.
  ask neighbs [
...
to-report get-discussion-capacity
  ...
  ;; Then we draw the value according the chosen method
  if Interest_Distribution = "constant" [
    set iValue Ratio_of_population_listened + random-float 0  ;; NOTE! 'random-float 0' is here for consuming one pseudorandom number to cunsume same number of pseudorandom numbers as "uniform
    if create_permanent_partners? [
      set iValue ifelse-value (iValue > (count my-links / (Number_Of_Agents - 1))) [iValue - (count my-links / (Number_Of_Agents - 1))][0]
    ]
  ]
  if Interest_Distribution = "normal" [
    set iValue precision (random-normal Ratio_of_population_listened Interest_STD) 3
...
"" ""
PENS
"default" 0.05 1 -16777216 true "" "histogram [discussion-capacity] of agents"
