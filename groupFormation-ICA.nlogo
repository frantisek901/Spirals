...
;; Here we'd like to apply HK model in more than 1D and look how agents adapt in >1D opinion space and whether they form groups.
;; THIS IS THE EXACT VERSION WHICH PRODUCES RESULTS FOR ICA ABSTRACT
;;

;; Created:  2021-10-21 FranCesko
;; Edited:   2021-11-06 FranCesko
...
to setup
  ...
  nw:generate-watts-strogatz turtles links N-agents n-neis p-random [
    ...
    set Opinion-position n-values opinions [precision (1 - random-float 2) 3]
...
to change-opinion-HK
  ...
  let influentials visibles with [opinion-distance <= (Uncertainty * sqrt(opinions * 4))]
  ...
  if count influentials > 0 [
      ...
      while [step < updating] [
        ...
        set opinion-position replace-item i opinion-position precision (mean [item i opinion-position] of influentials) 3 ;; NOTE: H-K model really assumes that agent adopts immediatelly the 'consesual' position
        set step step + 1
      ]
  ]
end
