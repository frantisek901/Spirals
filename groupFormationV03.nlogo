...
;; This is forked main model at 2021-11-26 with experimental feature: adding of one more dimension during concluding debate.
...
to setup
  ...
  ;; EXPERIMENTAL: Preparing for adding the opinion
  if add-opinion? [set opinions opinions + 1]
...
to record-state-of-simulation
  ...
  if ticks = 0 [
    ...
    foreach range (Opinions + ifelse-value (add-opinion?) [1][0]) [i -> file-write (word "Opinion" (i + 1) ", ")]   ;; and its Opinions
...
to go
  ...
  ;; Conditions for adding the one more opinion
  if add-opinion? [  ;; Active only for scenarios indicated by ADD-OPINION? == TRUE.
    if opinions < length [opinion-position] of turtle 0 [  ;; Rest will be activated only when one more opinion has not been added yet.
      if mean [mean Record] of turtles > adding or ticks > (max-ticks / 2) [  ;; Sub-routine is done once, when average of RECORD averages exceeds firstly 0.9. (Might be parametrized...)
        set opinions (opinions + 1)  ;; We add one more opinion ...
        if record? [record-state-of-simulation]  ;; ...and when we recording states of simulation, then we rocord the state at the time of adding one more opinion.
      ]
    ]
  ]

  ;; Finishing condition:
  if mean main-Record = 1 or max-ticks = ticks [
    if record? [record-state-of-simulation]
    if add-opinion? [set opinions (opinions - 1)]
    stop
  ]
...
;  ;; we store the number of opinion dimensions
;  let steps length my
...
278
file-name
Sims/Sims02_10_100_0.25_49_3_1_0.1_uniform_1_openly-listen.csv
1
0
String
